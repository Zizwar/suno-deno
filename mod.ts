    ...
    async saveSongs(songsInfo, outputDir) {
        try {
            ...
            for (let i = 0; i < songsInfo.length; i++) {
                ...
                // Save the info
                await Deno.writeTextFile(jsonPath, JSON.stringify(songInfo, null, 2));
                console.log("Info downloaded");

                // Save the lyric
                await Deno.writeTextFile(lrcPath, `${title}\n\n${lyric}`);
                ...
                const imageFileStream = await Deno.open(imagePath, { write: true, create: true });
                for await (const chunk of imageResponse.data) {
                    await Deno.write(imageFileStream.rid, chunk);
                }
                Deno.close(imageFileStream.rid);
                ...
                const fileStream = await Deno.open(mp3Path, { write: true, create: true });
                for await (const chunk of response.data) {
                    await Deno.write(fileStream.rid, chunk);
                }
                Deno.close(fileStream.rid);
